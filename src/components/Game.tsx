...
const getCountFlips = (
  ...
  if (
    isWithinBoard(x + i * dx, y + i * dy) &&
    squares[y + i * dy][x + i * dx] === player.squareType
  ) {
    return i - 1;
  }
  return 0;
...
export const Game = (props: { players: IPlayer[] }) => {
  const { players } = props;
  ...
  return (
    <>
      ...
      />
      <Option>
        <InitButton onClick={() => dispatch({ t: "init", player: players[0] })}>
          init
        </InitButton>
